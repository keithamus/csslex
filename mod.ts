...
for (let i = 0; i < 128; i += 1) {
  ranges[i] =
    ...
    (specialDelims[i as keyof typeof specialDelims] && SPECIAL_DELIM_RANGE) ||
...
const consumeURLSequence = (
  str: string,
  pos: number
): [typeof URL | typeof BAD_URL, number] => {
  let token: typeof URL | typeof BAD_URL = URL;
  const len = str.length;
...
export function* lex(str: string): Generator<Token> {
  const len = str.length;
  let token: (typeof types)[keyof typeof types];
  let start = 0;
  let pos = 0;
  while (pos < len) {
...
export const value = (str: string, [token, start, end]: Token) => {
  switch (token) {
    case COMMENT:
